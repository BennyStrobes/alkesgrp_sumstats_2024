...
def extract_dictionary_list_of_hapmap3_rsids(hapmap3_rsid_file, mhc_rsid_file):
	# Extract dictionary list of mhc rsids (to ignore)
	mhc_rsids = {}
	f = open(mhc_rsid_file)
	for line in f:
		line = line.rstrip()
		if line in mhc_rsids:
			print('repeat mhc rsid assumption oeroror')
			pdb.set_trace()
		mhc_rsids[line] = 1
	f.close()
	...
	for line in f:
		line = line.rstrip()
		if line.startswith('rs') == False:
			print('rsid assumption eroror')
			pdb.set_trace()
		if line in hm3_rsids:
...
def print_gazal_lab_summary_statistics_into_2024_sumstats_dir(gazal_lab_sumstats_dir, gazal_lab_sumstats_summary_file, hapmap3_rsids, alkesgrp_2024_sumstats_dir, t_summary_2024):
	...
	for trait_iter, trait_name in enumerate(trait_names):
		...
		t_summary_2024.write(trait_name + '\t' + trait_identifier + '\t' +  trait_origin + '\t' + trait_reference + '\t' + str(NN) + '\t' + str(MM) + '\t' + trait_population + '\t' + signed_sumstat + '\n')
...
def print_alkesgrp_2021_summary_statistics_into_2024_sumstats_dir(alkesgrp_2021_sumstats_dir, alkesgrp_2021_sumstats_summary_file, hapmap3_rsids, alkesgrp_2024_sumstats_dir, t_summary_2024):
...
mhc_rsid_file = sys.argv[6]  # exclude these rsids
...
t_summary_2024.write('Trait Name\tTrait Identifier\tOrigin\tReference\tN\tM\tPopulation\tSigned SumStat\n')

# Print alkesgrp 2021 summary statistics into alkesgrp 2024 sumstats dir
t_summary_2024 = print_alkesgrp_2021_summary_statistics_into_2024_sumstats_dir(alkesgrp_2021_sumstats_dir, alkesgrp_2021_sumstats_summary_file, hapmap3_rsids, alkesgrp_2024_sumstats_dir, t_summary_2024)
